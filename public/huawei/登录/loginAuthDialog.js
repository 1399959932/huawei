...
(function($, window, undefined) {
	...
	var verifyIDDialog = function(param) {
		...
		htmlStr += '<div style="width:100%;text-align:center;">'
				+ getNoAuthCodeLinkDiv('width:100%;text-align:center;margin: 20px auto 0;')
				+ '</div>';
	...
	function noReviceSecCodeDialog(userAccount, authCodeSentList, callback) {
		...
		if(secEmail&&secPhone)
		{
			noway_to_use=rss.noway_to_use_security_device
		}else if(secPhone)
		{
			
			noway_to_use = rss.resetPwd_inputSecAuthCode_unableUsePhone;
		}else if(secEmail)
		{

			noway_to_use = rss.resetPwd_inputSecAuthCode_unableUseEmail;
		}	
		var noReviceCodeHTml = "<div class='margin-t16-b16'><div>"
				+ rss.offline_device_problem
				+ "</div><div>"
				+ rss.getcode_onoffline
				+ "</div></div><div class='line-EMUI5 center' ><a class='a-EMUI5' target='_blank' href='"+localHttps+"/portal/appealSelf/applyChangeAccount.html"+localInfo.queryString+"&extInfo="+localInfo.extInfo+"'>"
				+ noway_to_use + "</a></div>";
